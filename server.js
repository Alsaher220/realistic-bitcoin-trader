...
const result = await pool.query(`SELECT w.id, w.amount, w.wallet, w.status, w.date, u.username FROM withdrawals w JOIN users u ON w.user_id = u.id ORDER BY w.date DESC`);
...
const result = await pool.query(`SELECT i.id, i.amount, i.plan, i.status, i.created_at, u.username FROM investments i JOIN users u ON i.user_id = u.id ORDER BY i.created_at DESC`);
res.json({ success: true, investments: result.rows });
}));

app.get(’/admin/topups’, verifyAdmin, asyncHandler(async (req, res) => {
const result = await pool.query(`SELECT t.id, t.user_id, t.amount, t.admin_id, t.date, u.username FROM topups t JOIN users u ON t.user_id = u.id ORDER BY t.date DESC`);
