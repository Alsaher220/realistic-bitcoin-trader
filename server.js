...
db.serialize(() => {
    ...
    // Sample users
    const users = [
        { username: 'alice', password: 'alice123', cash: 1000, btc: 0.5 },
        { username: 'bob', password: 'bob123', cash: 500, btc: 1.2 },
        { username: 'charlie', password: 'charlie123', cash: 2000, btc: 0 }
    ];
    ...
    users.forEach(user => {
        db.get(`SELECT * FROM users WHERE username = ?`, [user.username], (err, row) => {
            if (!row) {
                ...
                db.run(`INSERT INTO users (username, password, cash, btc) VALUES (?, ?, ?, ?)`,
                    [user.username, hashed, user.cash, user.btc]);
            }
        });
    });
});
